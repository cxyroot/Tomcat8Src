...
public abstract class AbstractEndpoint<S> {
    ...
    public abstract static class Acceptor implements Runnable {
        ...
        public final AcceptorState getState() {
            System.out.println("org.apache.tomcat.util.net.AbstractEndpoint.Acceptor.AcceptorState====3");
            return state;
        }
    ...
    public boolean processSocket(SocketWrapperBase<S> socketWrapper,
        ...
        System.out.println("org.apache.tomcat.util.net.AbstractEndpoint.processSocket");
        try {
            if (socketWrapper == null) {
                return false;
            }
            System.out.println("创建处理器"+"org.apache.tomcat.util.net.SocketProcessorBase");
            SocketProcessorBase<S> sc = processorCache.pop();
            if (sc == null) {
                System.out.println("org.apache.tomcat.util.net.NioEndpoint.createSocketProcessor ");
                sc = createSocketProcessor(socketWrapper, event);
            } else {
                sc.reset(socketWrapper, event);
            }
            System.out.println("java.util.concurrent.Executor");
            Executor executor = getExecutor();
            if (dispatch && executor != null) {
                //线程池
                System.out.println("java.util.concurrent.Executor.execute");
                executor.execute(sc);
            } else {
                sc.run();
            }
